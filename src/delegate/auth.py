"""
Authentication for DeleGate REST API.
...
def verify_api_key(
    ...
    the configured DELEGATE_API_KEY environment variable.
    ...
    if not settings.api_key:
        logger.error(
            ...
            "Set DELEGATE_API_KEY or enable DELEGATE_ALLOW_INSECURE_DEV=true (dev only)."
        )
        raise HTTPException(
...
def generate_api_key() -> str:
    """Generate a new API key with dg_ prefix.

    Utility function for generating keys - the key should be stored
    in DELEGATE_API_KEY environment variable.
    """
