"""DeleGate's binding to the shared MetaGate bootstrap client.
...
DeleGate-specific thing: which primitive types map to which settings.
...
_BINDING_SPECS: tuple[tuple[str, str], ...] = (
    ...
    ("asyncgate", "asyncgate_url"),
)

COMPONENT_KEY = "delegate"
...
async def bootstrap_from_metagate(settings: Any) -> Any:
    """Resolve DeleGate's peer endpoints from MetaGate."""
    if _shared is None:
        logger.warning(
